...
#cgo windows CFLAGS: -IC:/msys64/ucrt64/include/freerdp3
#cgo windows CFLAGS: -IC:/msys64/ucrt64/include/winpr3

#cgo windows LDFLAGS: -LC:/msys64/ucrt64/lib
#cgo windows LDFLAGS: -lfreerdp3
#cgo windows LDFLAGS: -lfreerdp-client3
#cgo windows LDFLAGS: -lwinpr3
...
func init() {
	// OpenSSL 3.x (used by FreeRDP 3.x) requires the 'legacy' provider for MD4/RC4 support,
	// which are strictly required for NTLM and Standard RDP Security.
	// We must tell OpenSSL where to find legacy.dll by setting OPENSSL_MODULES.
	if os.Getenv("OPENSSL_MODULES") == "" {
		if exePath, err := os.Executable(); err == nil {
			bundledPath := filepath.Join(filepath.Dir(exePath), "ossl-modules")
			if info, err := os.Stat(bundledPath); err == nil && info.IsDir() {
				os.Setenv("OPENSSL_MODULES", bundledPath)
				return
			}
		}
		// Fallback for local development
		fallbackPath := `C:\msys64\ucrt64\lib\ossl-modules`
		if info, err := os.Stat(fallbackPath); err == nil && info.IsDir() {
			os.Setenv("OPENSSL_MODULES", fallbackPath)
		}
	}
}
